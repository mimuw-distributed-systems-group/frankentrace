...
if __name__ != '__main__':
    ...
    class srd(SimpleNamespace):
        ...
        class Decoder:
            ...
            @abc.abstractmethod
            def decode(self, startsample=None, endsample=None, data=None):
                """
                Main entrypoint. May be called parameterless.

                If :attr:`inputs` is an id of a decoder, that it uses that OUTPUT_PYTHON specific type.
                Otherwise, list of tuples, such that `len(data) == endsample - startsample`. Pos 0 is samplenum.
                Pos 1 is bytes object, where each position is either 0 or 1 and represents a channel. Same order as
                :attr:`channels`.
                """
                pass

            def metadata(selfself, key, value):
                # key sigrokdecode.SRD_CONF_SAMPLERATE => sample rate in Hz
                pass
            ...
            def standalone(self):
                parser = argparse.ArgumentParser(self.name, description=self.desc)
                parser.add_argument('file', type=str)
                for op in getattr(type(self), 'options', {}):
                    parser.add_argument('--' + op['id'], help=op['desc'], default=op['default'])
            ...
            def register(self, kind):
                srd.glob_stream_cnt += 1
                return (srd.glob_stream_cnt, kind)

            def put(self, startsample, endsample, output_id, data):
                """
                ``data`` param depends on kind:

                OUTPUT_ANN : list with two items
                    1. annotation kind: index in :attr:`Decoder.annotations`
                    2. list of annotation strings for various display space - from longest to shortest.
                OUTPUT_PYTHON: : any Python object
                    The type is not specified.
                OUTPUT_BINARY : list with two items
                    1. binary kind: index in :attr:`decoder.binary`
                    2. :cls:`bytes` object
                OUTPUT_META : object
                    Object of a certain type as defined in call to :meth:`register`.

                """
                if output_id[1] == srd.OUTPUT_BINARY:
                    # TODO: super hiper
...
class Decoder(srd.Decoder):
    ...
    def handle_hardware(self, buf):
        ...
        if pid == 0:
            ...
            self.csv_payload = ("COMP:%d data: %s" % (comp, what), data)
            ...
            self.csv_payload = ("COMP:%d PC" % comp, "0x%08x" % addr)
            ...
            self.csv_payload = ("COMP:%d addr_16b" % comp, "0x%04x" % offset)
    ...
    def handle_software(self, buf):
        ...
        # XXX: we don't want special handling for csv output ...
    ...
    def handle_timestamp(self, buf):
        ...
        return [1, ['Timestamp: Δ%d (%s)' % (ts, msg), 'T: Δ%d (%s)' % (ts, msg),  'T:Δ%d%s' % (ts, '' if msg == 'exact' else '?')]]
...
if __name__ == '__main__':
    dec = Decoder().standalone()
    with open(dec.options['file'], 'rb') as f:
        data = f.read()
        initial_nulls = True
        for i, d in enumerate(data):
            if not d and initial_nulls:
                continue
            initial_nulls = False

            dec.decode(i, i+1, ('DATA', None, (d, None)))
